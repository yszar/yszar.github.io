---
title: 在Ubuntu 18.04系统上安装Python 3.8的两种方法
translate_title: two-ways-to-install-python-38-on-ubuntu-1804-system
tags:
  - linux
  - python
categories:
...
注：意思是按`[ENTER]`继续，或按`Ctrl-c`取消添加。
...
```
`--enable-optimizations`选项通过运行多个测试来优化Python二进制文件，这会使构建过程变慢。
...
$ make -j 8
```
为了加快构建时间，请修改`-j`以使其对应于处理器中的内核数，你可以通过键入`nproc`查找编号。
...
不要使用标准的`make install`，因为它将覆盖默认的系统python3二进制文件。
